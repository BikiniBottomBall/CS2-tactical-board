...
静态服务 + Pydantic 契约 REST API + WebSocket 多人协同 + 对齐校验图写盘
...
from auth import generate_token, get_or_create_user, validate_connection
...
@app.post("/api/utilities", response_model=UtilityOut)
def create_utility(data: UtilityCreate):
...
@app.put("/api/utilities/{uid}", response_model=UtilityOut)
def update_utility(uid: int, data: UtilityCreate):
    with Session(engine) as db:
...
@app.get("/api/tactics", response_model=list[TacticOut])
def list_tactics():
    with Session(engine) as db:
        return [
            tactic_to_dict(t, get_tactic_steps(db, t.id)) for t in db.exec(select(Tactic)).all()
        ]


@app.post("/api/tactics", response_model=TacticOut)
def create_tactic(data: TacticCreate):
...
@app.put("/api/tactics/{tid}", response_model=TacticOut)
def update_tactic(tid: int, data: dict = Body(...)):
    with Session(engine) as db:
            ...
            for i, s in enumerate(data["steps"]):
                ...
                db.add(
                    TacticStep(
                        ...
                        note=s.get("note"),
                        actors=(
                            json.dumps(s["actors"], ensure_ascii=False)
                            if s.get("actors") is not None
                            else None
                        ),
                        utility_ids=(
                            json.dumps(s["utility_ids"], ensure_ascii=False)
                            if s.get("utility_ids") is not None
                            else None
                        ),
        ...
        db.commit()
        db.refresh(t)  # commit 后实例过期，refresh 保证 model_dump 含全字段
...
def utility_content_hash(u: Utility) -> str:
    ...
    payload = {
        "name": u.name,
        "type": u.type,
        "throw_type": u.throw_type,
        "stand_x": u.stand_x,
        "stand_y": u.stand_y,
        "stand_z": u.stand_z,
        "landing_x": u.landing_x,
        "landing_y": u.landing_y,
        "landing_z": u.landing_z,
        "landing_point": u.landing_point,
...
def export_tactic_pack(tid: int):
    with Session(engine) as db:
        ...
        anns = [row_to_ann(r) for r in db.exec(select(Annotation)).all() if r.name in ann_names]
        for a in anns:
            a["_name"] = a.pop("name")
...
@app.post("/api/tactics/import", response_model=TacticOut)
def import_tactic_pack(pack: dict = Body(...)):
    if pack.get("format") != "cs2-tactic-pack":
        raise HTTPException(status_code=400, detail="not a cs2-tactic-pack")
    ...
    with Session(engine) as db:
        ...
        t = Tactic(
            name=name,
            description=(pack.get("tactic") or {}).get("description"),
            created_at=datetime.now().isoformat(timespec="seconds"),
        )
        db.add(t)
        db.commit()
...
def create_share(data: ShareCreate):
    """创建战术板分享链接，返回 share_id（body 为 {tactic_data: 战术包}）"""
    with Session(engine) as db:
        for _ in range(3):
            sid = uuid.uuid4().hex[:8]
            existing = db.exec(select(ShareLink).where(ShareLink.share_id == sid)).first()
            if not existing:
                break
        sl = ShareLink(
            share_id=sid,
            tactic_data=json.dumps(data.tactic_data, ensure_ascii=False),
            created_at=datetime.now().isoformat(timespec="seconds"),
        )
        db.add(sl)
        db.commit()
...
def get_share(share_id: str):
    """按 share_id 获取完整战术包"""
    with Session(engine) as db:
        sl = db.exec(select(ShareLink).where(ShareLink.share_id == share_id)).first()
        if not sl:
            raise HTTPException(status_code=404, detail="not found")
...
def view_share(share_id: str):
    ...
    if not os.path.exists(share_html):
        raise HTTPException(
            status_code=404, detail="share page not built, run: cd web && npm run build"
        )
...
def issue_token(anonymous_id: str):
    """签发 HMAC token（匿名鉴权：客户端持 anonymous_id 换 token）"""
    if not anonymous_id:
        raise HTTPException(status_code=422, detail="anonymous_id is required")
    return {"token": generate_token(anonymous_id)}


@app.post("/api/rooms")
async def api_create_room(data: RoomCreate):
    """创建房间。需 anonymous_id + token 验证。"""
    if not validate_connection(data.anonymous_id, data.token):
        raise HTTPException(status_code=401, detail="auth failed")
    get_or_create_user(data.anonymous_id, data.nickname or "")
    code = await create_room(data.anonymous_id, data.name or "")
    return {"code": code}
...
def api_join_room(code: str, data: RoomJoin):
...
async def api_close_room(code: str, data: RoomClose):
    """关闭房间（需 owner 验证）"""
    if not validate_connection(data.anonymous_id, data.token):
        raise HTTPException(status_code=401, detail="auth failed")
    room = get_room(code)
    if not room:
        raise HTTPException(status_code=404, detail="not found")
    if room.owner_id != data.anonymous_id:
        raise HTTPException(status_code=403, detail="only room owner can close")
    # 踢所有人；若房间本就无人，确保房间被销毁（leave_room 空房即销毁）
    for uid in list(room.players.keys()):
        await leave_room(code, uid)
    if get_room(code):
        await leave_room(code, data.anonymous_id)
    return {"ok": True}
...
def upload_demo(file: UploadFile = File(...)):
    if not file.filename or not file.filename.lower().endswith(".dem"):
        raise HTTPException(status_code=400, detail="只接受 .dem 文件")
    ...
    try:
        ...
        logger.exception("[demos] 解析失败: %s", raw_path)
    ...
    if is_new:
        with Session(engine) as db:
            for ev in events:
                db.add(
                    DemoEvent(
                        match_id=row.id, tick=ev["tick"], type=ev["type"], label=ev.get("label")
                    )
                )
            db.commit()
...
@app.get("/api/demos", response_model=list[MatchOut])
def list_demos():
    with Session(engine) as db:
...
def get_demo_pack(mid: int):
    with Session(engine) as db:
        ...
        path = m.file_parsed
    if not path:
        raise HTTPException(status_code=404, detail="no parsed file")
    abs_path = path if os.path.isabs(path) else os.path.join(ROOT, path)
    if not os.path.exists(abs_path):
        raise HTTPException(status_code=404, detail="parsed file missing")
...
async def room_websocket(ws: WebSocket, room_code: str):
    ...
    get_or_create_user(anonymous_id, nickname)
    ...
    try:
        ...
        logger.info("[ws] %s 断开（房间 %s）", anonymous_id, room_code)
    except Exception:
        logger.exception("[ws] 消息处理异常（房间 %s 用户 %s）", room_code, anonymous_id)
    finally:
