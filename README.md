...
CS2 de_dust2 官方地图 3D 战术板：真实模型加载、点位/区域标注、道具库（lineup 轨迹）、战术编排自动推演、demo 轨迹回放。
...
models.py         SQLModel 表结构（标注/道具/战术/demo 匹配/模型登记）
...
已完成：真实模型加载与纯色材质、标注系统（点/区域贴地投影）、道具库、战术编排、demo 回放、性能优化（索引合并 + 画质三档）、demo 解析管线（P0~P5 + P7）。
待做：P6 战术包分享、P8 链接分享、P9 多人协同。详见 `ROADMAP.md`。
